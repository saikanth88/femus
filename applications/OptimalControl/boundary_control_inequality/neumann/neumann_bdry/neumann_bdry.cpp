...
//========== compute gauss quantities on the boundary ================================================
		  ...
		  for(unsigned i_bdry=0; i_bdry < nDof_max_bdry; i_bdry++) {
		    unsigned int i_vol = msh->GetLocalFaceVertexIndex(iel, jface, i_bdry);
                 ...
                 double grad_u_dot_n_res = 0.;
//                 for(unsigned d=0; d < space_dim; d++) {
//                 grad_u_dot_n_res += sol_u_x_bdry_gss[d] * normal[d];  
//                 }
...
//============ Bdry Residuals ==================	
                Res[ assemble_jacobian<double,double>::res_row_index(Sol_n_el_dofs_Mat,pos_mat_state,i_vol) ] +=  - control_node_flag[i_vol] * penalty_ctrl * (  grad_u_dot_n_res - sol_eldofs_Mat[pos_mat_ctrl][i_vol] )
                    - control_node_flag[i_vol] *  weight_bdry * (grad_adj_dot_n_res * phi_u_bdry[i_bdry]);   // partial_u = q
                ...
                Res[ assemble_jacobian<double,double>::res_row_index(Sol_n_el_dofs_Mat,pos_mat_ctrl,i_vol) ]  +=  - control_node_flag[i_vol] *  weight_bdry *
							                           ...
							                           +  phi_ctrl_bdry[i_bdry]*sol_adj_bdry_gss );  //boundary optimality condition
                Res[ assemble_jacobian<double,double>::res_row_index(Sol_n_el_dofs_Mat,pos_mat_adj,i_vol) ]  +=                   - control_node_flag[i_vol] *  weight_bdry * phi_adj_bdry[i_bdry]*sol_ctrl_bdry_gss ; 
//============ Bdry Residuals ==================    
...
if ( i_vol == j_vol )  {
    double grad_u_dot_n_mat = 0.;
        for(unsigned d=0; d< space_dim; d++) {
	  grad_u_dot_n_mat += phi_u_x_bdry[j_bdry * space_dim + d] * normal[d];  //notice that the convention of the orders x y z is different from vol to bdry
	}
		Jac[ assemble_jacobian<double,double>::jac_row_col_index(Sol_n_el_dofs_Mat, sum_Sol_n_el_dofs, pos_mat_state, pos_mat_state, i_vol, j_vol) ] += penalty_ctrl * ( grad_u_dot_n_mat);
...
//==========block delta_control/adjoint ========
		     Jac[ assemble_jacobian<double,double>::jac_row_col_index(Sol_n_el_dofs_Mat, sum_Sol_n_el_dofs, pos_mat_ctrl, pos_mat_adj, i_vol, j_vol) ]  += 
		     control_node_flag[i_vol] * weight_bdry * phi_ctrl_bdry[i_bdry] * phi_adj_bdry[j_bdry];  
             
// THIRD BLOCK ROW             
//==========block delta_adjoint/control ========
		     Jac[ assemble_jacobian<double,double>::jac_row_col_index(Sol_n_el_dofs_Mat, sum_Sol_n_el_dofs, pos_mat_adj, pos_mat_ctrl, i_vol, j_vol) ]  += 
		     control_node_flag[i_vol] * weight_bdry * phi_adj_bdry[i_bdry] * phi_ctrl_bdry[j_bdry];  
